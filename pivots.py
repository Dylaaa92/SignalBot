        ...
        try:
            async with websockets.connect(WS_URL, ping_interval=20, ping_timeout=20) as ws:
                ...
                while True:
                        ...
                        # RUNNER: best-of stops + EMA cross exit + time stop
                            ...
                            if sig.side == "LONG":
                                ...
                                if closed["l"] <= runner_stop:
                                    ...
                                    time_exit = False
                                    if sig.tp1_t is not None:
                                        # bar-based is fine, but timestamp is bulletproof
                                        time_exit = (closed["t"] - sig.tp1_t) >= 60 * 60  # ~60 mins
                                    if ema_cross_exit:
                                ...
                                if closed["h"] >= runner_stop:
                                    ...
                                    time_exit = False
                                    if sig.tp1_t is not None:
                                        time_exit = (closed["t"] - sig.tp1_t) >= 60 * 60
                                    if ema_cross_exit:
                    ...
                    # ============================
                    if not sig.active:
                        # If bias/trend flips while waiting, drop setup (prevents stale arming)
