...
class Journal:
    ...
    def write_snapshot(self, snap: Dict[str, Any]) -> None:
        ...
        fieldnames = [
            "ts_utc","symbol","tf","open","high","low","close","volume",
            "ema9","ema21","vwap","poc","rsi","session_id"
        ]
        row = dict(snap)
    ...
    def write_decision(self, report: Any) -> None:
        ...
        # Build fieldnames dynamically the first time, but keep stable ordering:
        base_order = [
            "ts_utc","symbol","mode","strategy","data_fresh_ms",
            "px_last","spread_bps","vwap_5m","poc_5m","vwap_1h","poc_1h",
            "ema9_1h","ema21_1h","bias_1h","bias_reason","rsi_1h",
            "bos_dir","bos_level","retest_level","retest_state",
            "acceptance_bars","acceptance_required","acceptance_state",
            "vol_5m","vol_ma_20","vol_state","vol_reason",
            "entry_plan","entry_px","invalidation_px","stop_px","tp1_px",
            "runner_trail","rr_to_tp1",
    ...
    def write_trade(self, trade: Dict[str, Any]) -> None:
        ...
        fieldnames = [
            "ts_utc","symbol","side","qty","entry_px","stop_px","tp1_px",
            "exit_px","pnl_usd","pnl_r","reason","order_id","fill_id",
            "mode","session_id"
        ]
        row = dict(trade)
        row.setdefault("ts_utc", utc_now_iso())
